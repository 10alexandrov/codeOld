...
class PerformMoneySynchronizationEveryTime extends Command
{
     ...
     * @var string
     */
    protected $signature = 'prometeo:perform-money-synchronization-every-time {local_id?}';
    ...
    protected function connectToLocal(Local $local): void
    {
        ...
        try {
            ...
            try {
                ...
                DB::commit();
                $this->logSyncResult($local->id, 'Ok', 'Datos sincronizados correctamente');
                echo "Datos sincronizados correctamente.";
            } catch (Exception $e) {
                DB::rollBack();
                $this->logSyncResult($local->id, 'Fallo', 'Error al insertar los datos: ' . $e->getMessage());
                echo "Error al insertar los datos: " . $e->getMessage();
            }
        } catch (Exception $e) {
            $this->logSyncResult($local->id, 'Fallo', 'Error al conectar a la base de datos: ' . $e->getMessage());
    ...
    protected function logSyncResult($localId, $status, $message)
    {
        $syncLogLocal = SyncLogsLocals::where('local_id', $localId)->first();

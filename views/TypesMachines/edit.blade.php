    ...
    <div class="container">
        <div class="col-8 offset-2 isla-list p-4 mt-5">
            <div class="ttl text-center mb-4">
                <h1>Editar {{ $typeMachine->name }}</h1>
            </div>
            <form action="{{ route('typeMachines.update', $typeMachine->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="form-floating mb-3">
                    <input type="text" name="name" class="form-control" id="floatingName" placeholder="name" value="{{ old('name', $typeMachine->name) }}">
                                        ...
                                        @foreach ($zona->locals as $local)
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox"
                                                    ...
                                                    name="locals[]"
                                                    {{ in_array($local->id, $typeMachine->locals->pluck('id')->toArray()) ? 'checked' : '' }}>
                ...
                <div class="mt-5 mb-3">
                    <input type="submit" value="Actualizar Tipo de Máquina" class="btn btn-warning">

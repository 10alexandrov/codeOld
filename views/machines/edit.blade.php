                    ...
                    @foreach ($delegation->zones as $zona)
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="heading{{ $zona->id }}">
                                <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $zona->id }}" aria-expanded="true" aria-controls="collapse{{ $zona->id }}">
                                    {{ $zona->name }}
                                </button>
                            </h2>
                            <div id="collapse{{ $zona->id }}" class="accordion-collapse collapse" aria-labelledby="heading{{ $zona->id }}" data-bs-parent="#accordionExample">
                                <div class="accordion-body row">
                                    <div class="col-6">
                                        <h4>SALONES</h4>
                                        @foreach ($zona->locals as $local)
                                            <div class="form-check">
                                                <input class="form-check-input salon-radio" type="radio" value="S:{{ $local->id }}" name="local" {{ old('local', $machine->local_id ? 'S:' . $machine->local_id : '') == 'S:' . $local->id ? 'checked' : '' }}>
                                                <label class="form-check-label" for="radioSalon{{ $local->id }}">{{ $local->name }}</label>
                                            </div>
                                        @endforeach
                                    </div>
                                    <div class="col-6">
                                        ...
                                        @foreach ($zona->bars as $bar)
                                            <div class="form-check">
                                                <input class="form-check-input bar-radio" type="radio" value="B:{{ $bar->id }}" name="local" {{ old('local', $machine->bar_id ? 'B:' . $machine->bar_id : '') == 'B:' . $bar->id ? 'checked' : '' }}>
                                                <label class="form-check-label" for="radioBar{{ $bar->id }}">{{ $bar->name }}</label>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
